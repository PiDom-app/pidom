...
/**
 * `ScrollView`'s props, not `Box`'s.
 *
 * The CLI wrote `ComponentProps<typeof Box>` here and spread the result onto a
 * `ScrollView` below, so the inherited `ref` was typed for a `View` and `tsc`
 * refused the component outright. The props describe what they are spread onto;
 * this is the fix rather than a cast, because a `ref` on this really is a
 * scroll view's and a caller that takes one wants `scrollTo`.
 */
    ...
    <Tooltip
      ...
      trigger={(triggerProps) => {
        if (React.isValidElement(triggerElement)) {
          // `ReactElement.props` is `unknown` under React 19's types, so it
          // cannot be spread without saying what it is. The element is the
          // caller's own trigger and its props are a plain object.
