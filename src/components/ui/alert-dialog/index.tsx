...
const alertDialogBackdropStyle = tva({
  base: 'absolute left-0 top-0 right-0 bottom-0 bg-overlay/50 web:cursor-default',
});
...
>(function AlertDialogContent({ className, size, ...props }, ref) {
  ...
  return (
    <UIAccessibleAlertDialog.Content
      pointerEvents="auto"
      // `ZoomIn` only carries transform values — `opacity` was set here too but
      // is not part of the builder's config, so it was never applied. The
      // backdrop supplies the fade.
