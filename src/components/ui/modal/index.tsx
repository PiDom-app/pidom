...
>(function ModalContent({ className, size, ...props }, ref) {
  ...
  return (
    <UIModal.Content
      ref={ref}
      // `withInitialValues` on a `ZoomIn` takes transforms only — `opacity` is
      // not one, and the CLI's extra key made the whole builder refuse to
      // typecheck. The backdrop's own fade is what dims the entrance.
