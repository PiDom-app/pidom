...
in `app.config.ts` to link at all. Both are there now. They were not before, which
...
`app.config.ts` registers Pidom as a PDF handler on both platforms, which puts it in
...
Google Cloud then fails with _"the Android package name and fingerprint are
already in use"_ — Google requires every (package name, SHA-1) pair to be unique
...
`expo prebuild`, and _not_ with `~/.android/debug.keystore`. They are different
...
it appends an iOS URL scheme and does _nothing whatsoever for Android_ — no
...
is what `app.config.ts` does now, alongside `android.googleServicesFile`. The iOS half
...
system adds to every font weight at _render_ time. React Native measures text
...
`app.config.ts` and re-run `npx expo prebuild` to get the system behaviour back.
